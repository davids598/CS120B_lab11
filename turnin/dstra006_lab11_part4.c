 ...
 *
 * Link to Vid: https://youtu.be/5eDbBjVf5MY
 *
...
unsigned char i = 0;
unsigned short write_loc = 1;
unsigned char keypad_data;
...
int LCD_KeypadInputsSMTick(int state) {
  ...
  switch (state) {
    case Keypad_input:
      keypad_data = GetKeypadKey();
      break;
    case hold:
      keypad_data = GetKeypadKey();
      break;
    case display:
      LCD_Cursor(write_loc);
      LCD_WriteData(data_to_write);
      LCD_Cursor(write_loc + 1);
      if (write_loc >= 16) {
        write_loc = 1;
        LCD_Cursor(write_loc);
      } else {
